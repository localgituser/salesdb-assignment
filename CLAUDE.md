...
**SalesDB Regional Data Lead Assessment** — A PoC pipeline for market coverage audit and enrichment of a 4.25M-company dataset. The project executes in 6 sequential parts:

- **Part 0**: Ingestion (CSV → DuckDB → Parquet)
- **Part 1**: Baseline & Stratification (fill-rate audit, state tiering)
- **Part 2**: Agentic Audit (surface gaps via LLM)
- **Part 3**: Commercial Framing (ICE score top gaps)
- **Part 4**: PoC Enrichment Pipeline (cascade: rules → search → Haiku verify → Sonnet fallback)
- **Part 5**: Reusable Skill documentation

**Project parameters**: All tunable parameters (budget caps, gap-tier thresholds, geography tier cutoffs, platform blocklist, run scope, market dataset/comparator paths) live in `config/project.yaml` and are loaded via `src/shared/config.py` (`CONFIG`). To run this project against a new market, edit that YAML — no code changes required. **Never hardcode these values elsewhere.**

**Self-imposed budget ceiling**: $10 total — chosen as a discipline constraint, not a brief requirement. Cost is logged and checked per part. See `config/project.yaml` → `budget` for the authoritative split.

**Note on Part 6**: The 90-Day Pod Plan is a Notion deliverable, not a code part — no script in `src/` corresponds to it. It's written last, informed by outputs from Parts 1–4.
...
**Coverage audit workflow**: When running Part 1 (data-profiler) or Part 2 (data-engineer), consult `.claude/skills/coverage-audit/SKILL.md` for *workflow* definitions (stage roles, sampling strategy, enterprise weighting, coverage parity targets) and `config/project.yaml` for *numeric thresholds* (gap tier cutoffs, geography tier cutoffs, blocklist, run scope). The skill defers to the YAML when they disagree.

**Never let one agent do both roles for the same part.** The separation is the point — it's how trust calibration gets demonstrated (Part 2 of the brief).

Verifier calls (spot-checks, eval_runner) do not count against the part's LLM budget — they're read-only/deterministic by design.
...
### Part-Based Execution Model

Each part is a standalone script in `src/`. They run sequentially but independently — no shared state between them except through output files. Parts 0–1 are rules-based (no LLM); Parts 2–4 use LLM calls.

**Key files**:
- `src/part0_ingestion.py` — Load CSV into DuckDB, export to Parquet
- `src/part1_sampling.py` — Stratify by state/industry/size, tier states (A/B/C), candidate key analysis
- `src/shared/rules.py` — Deterministic cleanup (URL/domain, regex normalization)
- `src/part4_pipeline.py` — Cascade enrichment (rules → search → Haiku → Sonnet), with explicit retries/fallback state machine
- `src/shared/observability.py` — Cost tracking and logging
...
```python
logger = ObservabilityLogger()
phase_cost = logger.get_phase_cost("part_2")
if phase_cost > PART_2_BUDGET:
    logger.info(f"Part 2 budget exhausted. Cost: ${phase_cost:.4f}")
    sys.exit(1)
```

### Rules vs. LLM

**Rules-based enrichment (Part 1, Part 4 first stage):
- URL/domain cleanup (regex, TLD validation)
- Format normalization (phone, postal codes)
- Deduplication (exact match)

**LLM-based enrichment (Part 2 audit, Part 4 later stages):
...
```bash
python src/part1_sanity_check.py  # Quick state/industry/size distribution
```

### Part 4: Enrichment & Evaluation
```bash
...
- `docs/strategy_v3.md` — Full execution plan with budgets, timelines, and brief Part mapping
...
- `skills/` — Root-level symlink to `.claude/skills/` (submission-facing)

## Important Patterns & Constraints

### Dataset Metadata (Part 1 completed findings)

The 4.16M US record dataset (from Part 1 baseline analysis):
- **Primary key**: `handle` — perfectly unique (0 collisions, 0 nulls). Use as the stable entity identifier for all joins, logging, and merge-back across Parts 2–4.
...
### Candidate Key Analysis (Part 1 — completed)
...
### Part 2 Gap Detection — Context for data-engineer
...
Comparator source data: `src/part1_comparator.py` (SUSB), `src/part1_nes_comparator.py` (NES). Full analysis in `docs/part1-baseline.md` Sections 8–10.
...
Per-segment precision/recall in the Part 4 eval is the actionable signal — tells us which size bands the cascade is trustworthy for. The earlier two-run framing (51+ first, micro/SMB deferred) was discarded; see `docs/part1-baseline.md` §2b for rationale.

The enrichable size bands (all four segments) are listed in `config/project.yaml` → `markets.us.enrichable_size_bands`. The Part 4 batch-quality gate (`src/part4_gate.py::check_part4_entry`) uses this list to catch `size IS NULL` or unexpected band values — it no longer rejects sub-51 records.

### Part 4 Eval Thresholds — Context for verifier
...
Per-part caps live in `config/project.yaml` → `budget.per_part_usd` (total: `budget.total_usd`). Current split: Part 2 = $3 (agentic audit), Part 4 = $5 (enrichment cascade + evals), all others = $0.

If a part's budget is exhausted, **stop LLM calls and log it**, never carry over budget from a later part.

### Output Compliance

- All Markdown outputs (`baseline_audit.md`, `gap_findings.md`, etc.) must be **human-readable** and include:
  - Timestamp
  - Execution parameters (model, part, sample size)
...
logger.info(f"Part 1: loaded {record_count:,} records")
...
- **Part 0**: Sanity check (state codes, record counts vs. comparator)
- **Part 1**: Tier distributions and fill-rate tables
- **Part 4**: Precision/recall from `eval_runner.py` against ground_truth
...
### Run a single phase
```bash
python src/part4_pipeline.py  # Part 4
```
...
**Upstream Feedback Loop (Part 2 → Part 1)**: When agents discover a systematic programmatic bug (e.g., a whole vertical has trailing syntax like `"Technology, Inc."`), do not patch it inside the Part 2 agent script. Log the issue, route it back to `src/shared/rules.py`, re-ingest, and re-run from the clean state. Keep the diagnostic layer clean.

**Search Wrapper Failure (Part 4)**: Never use consumer-facing search wrappers (Perplexity, etc.) for structured pipeline throughput — they introduce latency variance and make strict JSON schema enforcement unreliable. Use a structured API (e.g. Google Places) to fetch raw strings, then pass explicitly to Claude for entity matching and schema-conforming output.

## Notes on Codebase State

- **Parts 0–1**: Core infrastructure (ingestion, observability, stratification) — stable, foundational
- **Parts 2–4**: Still under development (placeholder TODOs in `part1_sampling.py`, `shared/rules.py`, `part4_pipeline.py`, `eval_runner.py`)
- **Part 5**: Not started (will document reusable skill for audit or enrichment)

When extending, maintain the part-based structure and **always log LLM costs** if introducing new LLM calls.
