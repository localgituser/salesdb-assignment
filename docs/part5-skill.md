...
- **Traced**: every LLM call logged to `data/processed/observability.jsonl` with prompt_version, model, cost, latency, outcome
...
Inputs: data/processed/us_companies_clean.parquet, config/project.yaml (markets.us)
Run internal profiling first, then external gap detection vs. SUSB+NES.
Output gap_candidates.json and gap_findings.md.
```
